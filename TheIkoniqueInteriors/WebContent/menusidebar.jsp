<aside class="menu-sidebar d-none d-lg-block">
	<div class="logo">

		<a href="#"> <img src="images/icon/logo.png" />
		</a>
	</div>
	<div class="menu-sidebar__content js-scrollbar1">
		<nav class="navbar-sidebar">
			<ul class="list-unstyled navbar__list">
				<li class="active has-sub"><a class="js-arrow" href="#"> <i
						class="fas fa-tachometer-alt"></i>Dashboard
				</a>
					<ul class="list-unstyled navbar__sub-list js-sub-list">
						<li><a href="index.jsp">Dashboard 1</a></li>
						<!-- 								<li><a href="index2.jsp">Dashboard 2</a></li> -->
						<!-- 								<li><a href="index3.jsp">Dashboard 3</a></li> -->
						<!-- 								<li><a href="index4.jsp">Dashboard 4</a></li> -->
					</ul></li>
				<li><a href="chart.jsp"> <i class="fas fa-chart-bar"></i>Charts
				</a></li>
				<li><a href="table.jsp"> <i class="fas fa-table"></i>Tables
				</a></li>
				<li><a href="form.jsp"> <i class="far fa-check-square"></i>Forms
				</a></li>
				<li><a href="calendar.jsp"> <i class="fas fa-calendar-alt"></i>Calendar
				</a></li>
				<li><a href="map.jsp"> <i class="fas fa-map-marker-alt"></i>Maps
				</a></li>
				<li class="has-sub"><a class="js-arrow" href="#"> <i
						class="fas fa-copy"></i>Pages
				</a>
					<ul class="list-unstyled navbar__sub-list js-sub-list">
						<li><a href="login.jsp">Login</a></li>
						<li><a href="register.jsp">Register</a></li>
						<li><a href="forget-pass.jsp">Forget Password</a></li>
					</ul></li>
				<li class="has-sub"><a class="js-arrow" href="#"> <i
						class="fas fa-desktop"></i>UI Elements
				</a>
					<ul class="list-unstyled navbar__sub-list js-sub-list">
						<li><a href="button.jsp">Button</a></li>
						<li><a href="badge.jsp">Badges</a></li>
						<li><a href="tab.jsp">Tabs</a></li>
						<li><a href="card.jsp">Cards</a></li>
						<li><a href="alert.jsp">Alerts</a></li>
						<li><a href="progress-bar.jsp">Progress Bars</a></li>
						<li><a href="modal.jsp">Modals</a></li>
						<li><a href="switch.jsp">Switchs</a></li>
						<li><a href="grid.jsp">Grids</a></li>
						<li><a href="fontawesome.jsp">Fontawesome Icon</a></li>
						<li><a href="typo.jsp">Typography</a></li>
					</ul></li>
			</ul>
		</nav>
	</div>
</aside>